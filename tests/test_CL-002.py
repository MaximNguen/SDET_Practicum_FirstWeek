...
class TestSearchAndCartFunctionality:
    ...
    def _add_product_to_cart_from_search(self, position: int) -> None:
        ...
        assert len(cards) == 1, f"Не найдена карточка товара с позицией {position} для добавления в корзину."
    ...
    def test_check_add_to_cart_button(self):
        ...
        for position in [2, 3]:
            with allure.step(f"Добавляем в корзину товар из результата поиска с позицией {position}"):
